...
`fullsample-gmm` implements full-sample debiased GMM with user-supplied
...
Python 3.10 or newer is required. Download and extract the
`fullsample_gmm_dml` folder, open a terminal in that folder, and create an
isolated environment:

```sh
cd fullsample_gmm_dml
python3 -m venv .venv
...
The package's runtime dependencies are NumPy, pandas, SciPy, and
scikit-learn.

The downloaded folder is named `fullsample_gmm_dml`, the distribution
installed by pip is named `fullsample-gmm`, and Python code imports it as
`fullsample_gmm`:

```python
from fullsample_gmm import fit_gmm
```

For development, install the source in editable mode together with the test
dependency and run the tests:
...
```python
from fullsample_gmm import NuisanceSpec, sklearn_nuisance_fit
...
```python
from fullsample_gmm import MultiwayGroupKFold
