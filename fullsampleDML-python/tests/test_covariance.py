...
from fullsampleDML import compute_cluster_moment_matrix
...
def test_cross_language_cluster_moment_fixture():
    data = pd.DataFrame({"one": [0, 0, 1, 1], "two": [0, 1, 0, 1]})
    scores = np.array([[1, 2], [3, 4], [5, 6], [7, 8]], dtype=float)
    fixture = pd.read_csv(
        Path(__file__).parent / "fixtures" / "cluster_moments.csv"
    )
    for method in ("PSD", "CGM"):
        rows = fixture.loc[fixture["method"] == method]
        expected = rows["value"].to_numpy().reshape(2, 2)
        result = compute_cluster_moment_matrix(
            scores, data, ["one", "two"], method
        )
